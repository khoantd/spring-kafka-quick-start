...
When reviewing UI code (components, pages, layouts, styling, accessibility), read `.agents/skills/ui-ux-pro-max/SKILL.md` and verify the **Pre-Delivery Checklist**: SVG icons (no emoji), contrast, `cursor-pointer`, hover/focus states, responsive breakpoints, form labels, `prefers-reduced-motion`. Flag violations in review comments.
